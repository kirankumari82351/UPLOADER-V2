...
class Database:
    ...
    def _connect_with_retry(self, max_retries: int, retry_delay: float):
        ...
        for attempt in range(1, max_retries + 1):
            try:
                ...
                self.topics = self.db['forum_topics']
    ...
    def _create_indexes(self):
        ...
        try:
            self.topics.create_index(
                [("chat_id", 1), ("topic_name", 1)],
                unique=True,
                name="forum_topics_identity"
            )
            index_results.append("forum topics index")
        except Exception as e:
            print(f"{Fore.YELLOW}âš  Could not create forum topics index: {str(e)}{Style.RESET_ALL}")
    ...
    def get_forum_topic(self, chat_id: int, topic_name: str) -> Optional[int]:
        """Get saved forum thread id for a topic"""
        try:
            topic = self.topics.find_one(
                {"chat_id": int(chat_id), "topic_name": topic_name}
            )
            return topic.get("thread_id") if topic else None
        except Exception as e:
            print(f"Error getting forum topic: {str(e)}")
            return None

    def save_forum_topic(self, chat_id: int, topic_name: str, thread_id: int) -> bool:
        """Persist topic to forum thread mapping"""
        try:
            ...
            print(f"Error saving forum topic: {str(e)}")
            return False
